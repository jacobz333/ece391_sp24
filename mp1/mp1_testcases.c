...
int main(void)
{
	...
	// struct mp1_blink_struct test_struct;
	// test_struct.on_char = 'A';
	// test_struct.off_char = 'B';
	// test_struct.on_length = 1;
	// test_struct.off_length = 1;
	// test_struct.location = -1;
	// ret_val = ioctl(rtc_fd, RTC_ADD, (unsigned long) &test_struct);
	// test_struct.location = 2000;
	// ret_val = ioctl(rtc_fd, RTC_ADD, (unsigned long) &test_struct);

	// test_struct.location = 1000;
	// test_struct.on_char = 'C';
	// test_struct.off_char = 'D';
	// ret_val = ioctl(rtc_fd, RTC_ADD, (unsigned long) &test_struct);
	// test_struct.location = 1001;
	// test_struct.on_char = 'E';
	// test_struct.off_char = 'F';
	// ret_val = ioctl(rtc_fd, RTC_ADD, (unsigned long) &test_struct);
	// test_struct.on_char = 'G';
	// test_struct.off_char = 'H';
	// test_struct.location = 1002;
	// ret_val = ioctl(rtc_fd, RTC_ADD, (unsigned long) &test_struct);


	// // FIND TESTS
	// ret_val = ioctl(rtc_fd, RTC_FIND, (unsigned long) 0);

	// test_struct.on_char = 'A';
	// test_struct.off_char = 'B';
	// test_struct.location = 1002;
	// ret_val = ioctl(rtc_fd, RTC_FIND, (unsigned long) &test_struct);
	// test_struct.on_char = 'A';
	// test_struct.off_char = 'B';
	// test_struct.location = 1001;
	// ret_val = ioctl(rtc_fd, RTC_FIND, (unsigned long) &test_struct);
	...
	// test_struct.location = 999;
	// ret_val = ioctl(rtc_fd, RTC_FIND, (unsigned long) &test_struct);
	// test_struct.location = 1003;
	// ret_val = ioctl(rtc_fd, RTC_FIND, (unsigned long) &test_struct);
	// test_struct.location = -1;
	// ret_val = ioctl(rtc_fd, RTC_FIND, (unsigned long) &test_struct);
	// test_struct.location = 2000;
	...
	// // REMOVE TESTS
	// ret_val = ioctl(rtc_fd, RTC_REMOVE, (unsigned long) 0);
	// ret_val = ioctl(rtc_fd, RTC_REMOVE, (unsigned long) 2000);
	// ret_val = ioctl(rtc_fd, RTC_REMOVE, (unsigned long) 999);
	// ret_val = ioctl(rtc_fd, RTC_REMOVE, (unsigned long) 1004);
	// ret_val = ioctl(rtc_fd, RTC_REMOVE, (unsigned long) 1001);
	// ret_val = ioctl(rtc_fd, RTC_REMOVE, (unsigned long) 1002);
	// ret_val = ioctl(rtc_fd, RTC_REMOVE, (unsigned long) 1000);

	// // find on empty list
	// test_struct.location = 1999;
	// ret_val = ioctl(rtc_fd, RTC_FIND, (unsigned long) &test_struct);

	// // SYNC TESTS
	// // sync on empty lists
	// ret_val = ioctl(rtc_fd, RTC_SYNC, (unsigned long) ((9 << 16) | (7)));
	// ret_val = ioctl(rtc_fd, RTC_SYNC, (unsigned long) ((7 << 16) | (9)));
	// // sync on one node list
	// test_struct.location = 1000;
	// test_struct.on_char = 'C';
	// test_struct.off_char = 'D';
	// test_struct.on_length = 6;
	// test_struct.off_length = 8;
	// ret_val = ioctl(rtc_fd, RTC_ADD, (unsigned long) &test_struct);
	// ret_val = ioctl(rtc_fd, RTC_SYNC, (unsigned long) ((9 << 16) | (7)));
	// ret_val = ioctl(rtc_fd, RTC_SYNC, (unsigned long) ((7 << 16) | (9)));
	// ret_val = ioctl(rtc_fd, RTC_SYNC, (unsigned long) ((0 << 16) | (1000)));
	// ret_val = ioctl(rtc_fd, RTC_SYNC, (unsigned long) ((1000 << 16) | (0)));
	// // sync on two node list
	// test_struct.location = 1001;
	// test_struct.on_char = 'C';
	// test_struct.off_char = 'D';
	// test_struct.on_length = 3;
	// test_struct.off_length = 5;
	// ret_val = ioctl(rtc_fd, RTC_ADD, (unsigned long) &test_struct);
	// ret_val = ioctl(rtc_fd, RTC_SYNC, (unsigned long) ((9 << 16) | (7)));
	// ret_val = ioctl(rtc_fd, RTC_SYNC, (unsigned long) ((7 << 16) | (9)));
	// ret_val = ioctl(rtc_fd, RTC_SYNC, (unsigned long) ((0 << 16) | (1000)));
	// ret_val = ioctl(rtc_fd, RTC_SYNC, (unsigned long) ((1000 << 16) | (0)));
	// ret_val = ioctl(rtc_fd, RTC_SYNC, (unsigned long) ((1001 << 16) | (1000)));
	// // empty list
	// ret_val = ioctl(rtc_fd, RTC_REMOVE, (unsigned long) 1001);
	// ret_val = ioctl(rtc_fd, RTC_REMOVE, (unsigned long) 1000);
